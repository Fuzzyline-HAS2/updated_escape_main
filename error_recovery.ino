...
// 1) 모터 timeout 플래그 → 즉시 LatchSystemFault (safe stop)
// 2) bad event 누적 → 3사이클 연속 시 RecoverBeetleConnection
// ※ Beetle silence(무수신)는 bad event로 간주하지 않음.
// ---------------------------------------------------------
void HandleRuntimeRecovery() {
    // --- 모터 timeout: 즉시 safe stop ---
    ...
    // 외부에서 카운터가 감소했다면(ResetBeetleErrorCounters 호출) prev를 맞춰줌
