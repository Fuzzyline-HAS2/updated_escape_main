...
class TestFaultRetryable:
    ...
    def test_fault_clears_on_successful_activate(self):
        ...
        sm = EscapeMainSM()
        self._latch_via_motor_timeout(sm)
        sm.data_changed(game_state="activate")  # ClearSystemFault + 성공
    ...
    def test_fault_relatches_on_second_motor_timeout(self):
        ...
        sm.data_changed(game_state="activate")  # ClearSystemFault → timeout → MarkTransitionFailed
        sm.handle_runtime_recovery()            # LatchSystemFault again
        assert sm.system_fault_latched is True

    def test_stable_updates_after_successful_retry(self):
        """fault 이후 재시도 성공 시 stable이 정상적으로 갱신된다."""
        sm = EscapeMainSM()
        self._latch_via_motor_timeout(sm)
        assert sm.stable_game_state == "setting"

        sm.data_changed(game_state="activate")  # 재시도 성공
        assert sm.stable_game_state == "activate"
        assert sm.stable_door_target == "open"
