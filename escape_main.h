...
//****************************************Stable State
//Snapshot****************************************************************
enum StableMode  { MODE_WAIT, MODE_TAGCOUNT };
enum DoorTarget  { DOOR_UNKNOWN, DOOR_OPEN, DOOR_CLOSED };

struct RuntimeSnapshot {
    String        stableGameState;
    String        stableDeviceState;
    bool          stableRelayHigh;
    bool          stableGameTimerEnabled;
    StableMode    stableMode;
    DoorTarget    stableDoorTarget;
    unsigned long stableSinceMs;
    bool          stableValid;
};

RuntimeSnapshot gStableState;                  // 마지막 안정 상태 스냅샷
bool            transitionInProgress = false;  // 전이 진행 중 플래그
String          pendingGameState     = "";     // 전이 중인 game_state
String          pendingDeviceState   = "";     // 전이 중인 device_state
String          lastRecoveryReason   = "";     // 마지막 복구 사유
//****************************************Recovery
//Functions****************************************************************
